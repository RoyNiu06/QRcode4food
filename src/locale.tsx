...
const translations: Record<string, [string, string]> = {
  ...
  抽签彩蛋: ["抽籤彩蛋", "Draw surprise"],
  "连续抽签五次后，随机送上一句小小调侃。": ["連續抽籤五次後，隨機送上一句小小調侃。", "After five draws in a row, show a playful nudge."],
  启用第五次抽签彩蛋: ["啟用第五次抽籤彩蛋", "Enable the five-draw surprise"],
  "每行一句，最多 12 句；展示时会从当前语言的文案中随机挑选。": ["每行一句，最多 12 句；展示時會從目前語言的文案中隨機挑選。", "One line per message, up to 12. A random line is shown in the visitor's language."],
  简体中文文案: ["簡體中文文案", "Simplified Chinese messages"],
  繁体中文文案: ["繁體中文文案", "Traditional Chinese messages"],
  英文文案: ["英文文案", "English messages"],
  保存彩蛋设置: ["儲存彩蛋設定", "Save surprise settings"],
  抽签彩蛋设置已保存: ["抽籤彩蛋設定已儲存", "Draw surprise settings saved"],
  点击圆圈开始抽签: ["點擊圓圈開始抽籤", "Tap the circle to draw"],
  再点圆圈抽一次: ["再點圓圈抽一次", "Tap the circle to draw again"],
  "点一下，交给运气": ["點一下，交給運氣", "Let luck decide"],
  知道啦: ["知道啦", "Got it"],
  还没决定吗: ["還沒決定嗎？", "Still deciding?"],
  这次就定它: ["這次就定它", "Let's choose this one"],
  一起补充: ["一起補充", "Contribute"],
  餐厅资料: ["餐廳資料", "Restaurant details"],
  "餐厅资料、点餐入口和多个窗口，都可以在这里补充。审核通过后才会公开。": ["餐廳資料、點餐入口和多個窗口，都可以在這裡補充。審核通過後才會公開。", "Add restaurant details, ordering links and counters. Changes appear only after review."],
  不补充主入口: ["不補充主入口", "No main link update"],
  可一次补充多个窗口: ["可一次補充多個窗口", "Add several counters at once"],
  没有主二维码: ["沒有主二維碼", "No main QR code"],
  无图片: ["沒有圖片", "No image"],
  审核通过时发布此窗口: ["審核通過時發佈此窗口", "Publish this counter on approval"],
  点餐方式备注: ["點餐方式備註", "Ordering notes"],
  "点击跳转 App": ["點擊跳轉 App", "Opens an app"],
  可用微信小程序: ["可用微信小程式", "WeChat mini program"],
  其他说明: ["其他說明", "Other note"],
  繁体其他说明: ["繁體其他說明", "Traditional Chinese note"],
  英文其他说明: ["英文其他說明", "English note"],
  "例如：堂食扫码后在小程序完成点餐": ["例如：堂食掃碼後在小程式完成點餐", "e.g. Complete ordering in the mini program"],
...
export function localizedRestaurant(
  ...
  field: "name" | "address" | "description" | "other_note",
  locale: Locale,
): string {
